...
<p class="interfaces">
	<small>Implements: 
	<?php for ($i = 0, $split = FALSE, $count = count($interfaces); $i < $count; $i++, $split = " | "): ?>
		<?php echo $split.HTML::anchor($route->uri(array('class' => $interfaces[$i])), $interfaces[$i], NULL, NULL, TRUE) ?>
	<?php endfor ?>
	</small>
</p>
<?php endif ?>
...
<p class="note">
	This class is a transparent base class for 
	<?php echo HTML::anchor($route->uri(array('class'=>$child)), $child) ?>
...
<p class="note">
	...
	Class declared in <tt><?php echo Debug::path($path) ?></tt>
	on line <?php echo $doc->class->getStartLine() ?>.
	<?php else: ?>
	Class is not declared in a file, it is probably an internal 
	<?php echo HTML::anchor('http://php.net/class.'.strtolower($doc->class->name), $doc->class->name) ?>.
	<?php endif ?>
</p>
...
<dl>
	<?php foreach ($properties as $prop): ?>
	<dt>
		<h4 id="property:<?php echo $prop->property->name ?>">
			<?php echo $prop->modifiers ?> <code><?php echo $prop->type ?></code> $<?php echo $prop->property->name ?>
		</h4>
	</dt>
	<dd><?php echo $prop->description ?></dd>
	<dd><?php echo $prop->value ?></dd>
	<?php if ($prop->default !== $prop->value): ?>
	<dd><small>Default value:</small><br /><?php echo $prop->default ?></dd>
	<?php endif ?>
	<?php endforeach ?>
	</dl>
</div>
...
<div class="methods">
<?php foreach ($methods as $method): ?>
	<?php echo View::factory('userguide/api/method', array('doc' => $method, 'route' => $route)) ?>
<?php endforeach ?>
</div>
<?php endif ?>
