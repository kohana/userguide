...
Все функции обратного вызова (callback) определяются как имя поля и функция (используя синтаксис [PHP callback](http://ru2.php.net/callback)):

    $object->callback($field, $callback);

[!!] В отличие от фильтров и правил, функции обратного вызова не принимают входных параметров.
