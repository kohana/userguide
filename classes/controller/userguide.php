...
class Controller_Userguide extends Controller_Template {
	...
	public function before()
	{
		...
		else
		{
			...
			if (isset($_GET['lang']))
			{
				...
				// LoI18nad the accepted language list
			...
			I18n::$lang = Cookie::get('userguide_language');
	...
	public function action_docs()
	{
		...
		if ( ! $file = $this->file($module.'/'.I18N::$lang.'/'.$page))
		{
	...
	protected function _get_all_menu_markdown()
	{
		...
		if (empty($markdown))
		{
			// Get menu items
			$module = $this->request->param('module');
			if ( ! $file = $this->file($module.'/'.I18N::$lang.'/menu'))
			{
				$file = $this->file($module.'/menu');
			}
