...
/**
 * Controller for user guide and API browser.
 *
...
abstract class Kohana_Controller_Userguide extends Controller_Template {
	...
	public function index()
	{
		...
		// Don't show Disqus comments on the index page
	...
	public function error($message)
	{
		...
		// Don't show Disqus comments on error pages
		$this->template->show_comments = FALSE;

		// If we are in a module and that module has a menu, show that
		if (
			$module = $this->request->param('module') AND 
			$menu = $this->file($module.'/menu') AND 
			Kohana::$config->load('userguide.modules.'.$module.'.enabled')
		)
		{
		...
		else
		{
			...
			if ($_class->class->getParentClass() AND $_class->class->getParentClass()->name == 'Kodoc_Missing')
			{
				return $this->error(
					...
					.'Check your url and make sure that the module with that class is enabled.'
				);
			}
			...
			{
				return $this->error(
					'That class is in package that is hidden. '
					.'Check the <code>api_packages</code> config setting.'
				);
			}
	...
	/**
	 * Locates the appropriate markdown file for a given guide page. 
	 * Page URLs can be specified in one of three forms:
	 *
	 *  * userguide/adding
	 *  * userguide/adding.md
	...
	{
		$info = pathinfo($page);
		if (isset($info['extension']) AND in_array($info['extension'], array('md','markdown')))
		{
	...
	protected function _get_all_menu_markdown()
	{
		...
		if (empty($markdown))
		{
			...
			{
				// Add spans around non-link categories. 
